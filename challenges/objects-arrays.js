...
*/
function dinosaursType(name, diet, weight, length, period) {
	return {
		name: name,
		diet: diet,
		weight: weight,
...
const unisWithUni = [];
for (let i = 0; i < graduates.length; i++) {
	let allUnis = graduates[i].university;
	unisWithUni.push(allUnis);
}
...
function animalClass(zooAnimals) {
	const eachAnimal = zooAnimals.forEach(function(animal_name) {
		return eachAnimal.push(displayNames);
	});
}
const displayNames = [];
...
lowCaseAnimalNames.map((zooAnimals) => {
...
function populationTotal(zooAnimals) {
	const reduce = (accum, item) => {
		return accum + item;
	};
	const cb = zooAnimals.reduce(reduce, 0);
